...
# ==== TEMP: turn these on for debugging ====
DEBUG = False            # set True for one run to see probes
MIN_EDGE_MS = 0.0        # 0.0 for probe (no filtering)
SHOW_PROBE = True        # print a few neighbors for a few nodes
PROBE_NODES = 100000         # how many nodes to sample
PROBE_NEIGHBORS = 500000     # how many neighbors per sampled node
# ===========================================

# How to collapse latency ranges like prop_range=(min_ms,max_ms): "min"|"mid"|"max"
WEIGHT_FROM_RANGE = "mid"

# ---- SSSP algo toggle: "auto" (default), "binary", or "radix"
SSSP_ALGO = os.getenv("VL_SSSP_ALGO", "binary").strip().lower()
...
# Cache nid<->node mappings per topology
...
def _ensure_resolver(ftopo) -> Dict[Any, Any]:
    ...
    # Bonus: if topology exposes a way to fetch node by id, record a marker
...
def _edge_to_pair(e) -> Optional[Tuple[NodeID, float]]:
    ...
    # Object-like: find destination and weight
    dst = None
    for name in _DST_FIELDS:
        if hasattr(e, name):
            dst = getattr(e, name)
            dst = _call_if_callable(dst)
            break
    if dst is not None and hasattr(dst, "nid"):
        dst = getattr(dst, "nid")

    # Weight: prefer scalar latency/prop fields
    w = None
    for name in _WEIGHT_FIELDS:
        if hasattr(e, name):
            w = getattr(e, name)
            w = _call_if_callable(w)
            break

    # If only a range exists, collapse it
    if w is None:
        for rname in _RANGE_FIELDS:
            if hasattr(e, rname):
                rng = getattr(e, rname)
                rng = _call_if_callable(rng)
...
def _get_node_obj(ftopo, x):
    """Return a node object if possible, else return x as-is."""
...
def _extract_ring_idx(ftopo, node_id_or_obj) -> Tuple[Optional[int], Optional[int]]:
    """
    Try to extract (ring_id, sat_index). Returns (None, None) if unknown.
    """
    ...
    # 1) attribute-based on node object
    for a in _RING_ATTR_CANDIDATES:
        if hasattr(nobj, a):
            try:
                ring_id = int(getattr(nobj, a))
                break
            except Exception:
                pass
    ...
    if (ring_id is None or sat_idx is None) and hasattr(nobj, "nid"):
        ...
        for a in _INDEX_ATTR_CANDIDATES:
            if hasattr(nid, a):
                try:
                    sat_idx = int(getattr(nid, a))
                    break
                except Exception:
                    pass

    # 3) string fallback parse: e.g. "ring_3_sat_42" or "R3-S42"
...
def _is_future_neighbor(ftopo, u_id, v_id) -> bool:
    """
    Forward-only rule:
      - If ring/index unknown for either, allow.
      - If rings differ, allow (inter-ring edges).
      - If same ring, allow only if v.index > u.index.
    """
    ...
    if isinstance(raw, dict):
        ...
        for v, w in raw.items():
            ...
            try:
                ...
                continue
            if w > MIN_EDGE_MS:
                if forward_only and not _is_future_neighbor(ftopo, u, v):
                    continue
                out.append((v, w))
        return out
...
def _pick_algo(ftopo) -> Tuple[str, int]:
    """
    Decide which SSSP engine to use.
    Returns (algo_name, scale), where algo_name in {'binary','radix'}.
    In 'auto', prefer radix only if weights are non-negative and integer-ish.
    """
    if SSSP_ALGO == "binary":
        return ("binary", 1)
    if SSSP_ALGO == "radix":
        ok, scale = _maybe_integerish_weights(ftopo)
        return ("radix", (scale if ok and scale > 0 else 1))
    # auto
    ok, scale = _maybe_integerish_weights(ftopo)
    return (("radix", scale) if ok else ("binary", 1))
...
def _slo_key(slo_ms: Optional[float]) -> Optional[int]:
    if slo_ms is None:
        return None
    # bucket by external config to avoid exploding cache cardinality
    return getattr(config, "SLO_MS_TASK", None)


# --- Cached pair shortest path ------------------------------------------------
...
def _dijkstra_tree_binary(
    ...
    *,
    ...
    """
    Classic Dijkstra (binary heap) computing distances from src to ALL reachable nodes.
    Respects slo_ms as an upper bound (prunes relaxations that exceed it).
    Returns (dist_map, parent_map).
    """
    dist: Dict[NodeID, float] = {src: 0.0}
    parent: Dict[NodeID, Optional[NodeID]] = {src: None}
    pq: List[Tuple[float, NodeID]] = [(0.0, src)]
...
def _dijkstra_tree_radix(
    ...
    *,
    ...
    """
    Radix-heap Dijkstra computing distances from src to ALL reachable nodes.
    Only valid for non-negative integer-ish weights under the given scale.
    Returns (dist_map, parent_map) in float units.
    """
    INF = (1 << 62)
...
def _single_source_sssp(
    ...
    *,
    ...
    """
    One SSSP from src to ALL reachable nodes (tree), honoring slo_ms pruning.
    Returns (dist_map, parent_map).
    """
    algo, scale = _pick_algo(ftopo)
    if algo == "radix":
...
def _path_from_tree(
    ...
    """Convenience wrapper to reconstruct path from a cached SSSP tree."""
